...
class BaseConfig
{
    ...
    public function recordSysLog($code, $message, $file, $line, $from) 
    {
        ...
        if(in_array($code, $codes) &&  is_dir(Constant::getConstant('TEMP_PATH')))
        {
            ...
            if(is_callable(array($this, 'filterSysLog')) && 
                false === call_user_func_array(array($this, "filterSysLog"), array($logInfo))) {
                $isRecord = false;
            }
            if(true === $GLOBALS['STOP_SYS_LOG']) {
                $isRecord = false;
            }
            if($logInfo['query_sql']) {
                $message .= '('.$logInfo['query_sql'].')';
            }
    ...
    public function setRedisConfigs() {
        $this->redisConfigs = Constant::getConstant('REDIS_CONFIGS') 
            ? (
                \Doba\Util::isJson(Constant::getConstant('REDIS_CONFIGS'))
                ? \Doba\Util::dJson(Constant::getConstant('REDIS_CONFIGS'))
                : Constant::getConstant('REDIS_CONFIGS')
            )
            : array('default'=>array('host'=>'127.0.0.1', 'port'=>'6379', 'pass'=>'', 'persistent'=>false));
    }
    ...
    public function setDbConfigs() {
        $this->dbConfigs = Constant::getConstant('DB_CONFIGS')
            ? (
                \Doba\Util::isJson(Constant::getConstant('DB_CONFIGS')) 
                ? \Doba\Util::dJson(Constant::getConstant('DB_CONFIGS'), true)
                : Constant::getConstant('DB_CONFIGS') 
            )
            : array('default'=>array('dbHost'=>'127.0.0.1', 'dbName'=>'test', 'dbUser'=>'root', 'dbPass'=>''));
    }
    ...
    public function apiCall($api, $edatas=array(), $options=array())
    {
        ...
        {
            $rpcHost = Constant::getConstant('RPC_HOST');
            if(! $rpcHost) return false;
            $url = preg_match('/^https?:\/\//', $rpcHost) ? $rpcHost : "http://{$rpcHost}/rpc.php";    

            $_API_KEY = Constant::getConstant('API_KEY'); $_API_SECURE = Constant::getConstant('API_SECURE'); 
            $_LANGUAGE = Constant::getConstant('LANGUAGE') ? Constant::getConstant('LANGUAGE') : 'en';
        }
        $plus = $files = array();
    ...
    public function url($a, $plus="") {
        ...
        return Constant::getConstant('URL').'?a='.$a.($plus ? "&".preg_replace('/^[?|&]/', '', $plus) : '');
    }
    ...
    {
        $webPath = Constant::getConstant('WEB_PATH'); $pagePath = Constant::getConstant('PAGE_PATH');
        $langPath = Constant::getConstant('LANGUAGE_PATH'); $lang = Constant::getConstant('LANGUAGE');
        $cachePage = Constant::getConstant('CACHE_PATH').'page/'.basename($webPath).'/'.$lang.'/'.$page;
        if( 
            (is_array($GLOBALS['I18N_LANGS']) && count($GLOBALS['I18N_LANGS']) > 0)
            &&
            (
                ! is_file($cachePage) 
                || filemtime($cachePage) < filemtime($pagePath.$page)
                || filemtime($cachePage) < filemtime($langPath.$lang.'.php')
                || ($GLOBALS['COMMON_LANG_FILE'] && filemtime($cachePage) < filemtime($GLOBALS['COMMON_LANG_FILE']))
            )
        )
        {
            $sourceContent = file_get_contents($pagePath.$page);
        ...
        return is_file($cachePage) ? $cachePage : $pagePath.$page;
    }
}